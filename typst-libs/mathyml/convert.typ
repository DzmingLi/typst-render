...
#let _to-mathml(inner, ctx) = {
  ...
  if type(inner) == content {
    ...
    if func == math.equation {
      ...
      // Unwrap styled elements: detect style changes and apply to context
      let child = inner.child
      let new-ctx = ctx
      // Inspect the styled wrapper by comparing known style functions
      // We check the repr of styles to detect bold/italic/upright/variant changes
      let styles-repr = repr(inner.styles)
      if "bold" in styles-repr {
        new-ctx.styles.bold = true
      }
      if styles-repr.contains("italic") and not styles-repr.contains("italic: false") {
        new-ctx.styles.upright-or-italic = "italic"
      }
      // upright/normal: functions like upright() and dif set italic to false
      if styles-repr.contains("italic: false") or styles-repr.contains("upright") {
        new-ctx.styles.upright-or-italic = "upright"
      }
      // Variant detection for special math alphabets
      if "double-struck" in styles-repr or "bb" in styles-repr {
        new-ctx.styles.variant = "double-struck"
      } else if "fraktur" in styles-repr or "frak" in styles-repr {
        new-ctx.styles.variant = "fraktur"
      } else if "script" in styles-repr or "cal" in styles-repr {
        new-ctx.styles.variant = "script"
      } else if "sans-serif" in styles-repr or "sans" in styles-repr {
        new-ctx.styles.variant = "sans-serif"
      } else if "monospace" in styles-repr or "mono" in styles-repr {
        new-ctx.styles.variant = "monospace"
      }
      let inner-rec = _create-rec(new-ctx)
      inner-rec(child)
